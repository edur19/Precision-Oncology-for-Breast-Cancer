...
from sklearn.manifold import TSNE  # 🔄 CAMBIO: t-SNE en lugar de PCA
...
OUT_DIR = "resultados_normas_pareto_tsne" # Directorio de salida actualizado
...
# ============================================================
# 4️⃣ Reducción t-SNE con Múltiples Semillas y Grid Search 🔑
# ============================================================
embedding_matrices = {} 
X_input = X_scaled

# 🔄 CAMBIO: Parámetros para t-SNE
n_components_list = [2, 3]
perplexity_list = [30, 50]
learning_rate_list = [200, 'auto']

tsne_param_grid = []
for n_comp in n_components_list:
    for perp in perplexity_list:
        for lr in learning_rate_list:
            tsne_param_grid.append({
                'n_components': n_comp,
                'perplexity': perp,
                'learning_rate': lr
            })

print(f"\n🔸 Ejecutando t-SNE en X_scaled ({X_scaled.shape[1]} features) con {len(tsne_param_grid)} configuraciones y {len(SEEDS_TO_TEST)} semillas...")
...
for current_seed in SEEDS_TO_TEST:
    ...
    for params in tsne_param_grid:
        ...
        lr_str = 'auto' if learning_rate == 'auto' else str(learning_rate)
        emb_name = f'TSNE_C{n_components}_P{perplexity}_LR{lr_str}_S{current_seed}'

        try:
            # 🔧 CORRECCIÓN: Usar max_iter en lugar de n_iter
            tsne_model = TSNE(**params, random_state=current_seed, max_iter=1000, verbose=0)
            X_tsne = tsne_model.fit_transform(X_input)
            embedding_matrices[emb_name] = X_tsne
            print(f"   ✓ {emb_name} completado")
            
        except Exception as e:
            print(f"Error al ejecutar t-SNE con {emb_name}: {e}")
            continue
...
# 🔧 CORRECCIÓN: Verificar si hay datos antes de filtrar
if len(df_scores) == 0:
    print("\n❌ ERROR: No se generaron resultados de clustering. Verifica que los embeddings se hayan creado correctamente.")
    print("Posibles causas:")
    print("  • t-SNE no convergió para ninguna configuración")
    print("  • Todos los algoritmos fueron descartados por alto ruido")
    print("  • Error en los parámetros de t-SNE")
else:
    ...
    # ============================================================
    # 12️⃣ PLOTEO DEL MEJOR t-SNE 2D/3D 🖼️
    # ============================================================
    ...
    def generate_tsne_plots(df_top_models, embedding_matrices, output_dir):
        """Genera y muestra las gráficas de t-SNE para un subconjunto de los modelos."""

        if df_top_models.empty:
            print("\n⚠️ No hay modelos TOP para graficar.")
            return

        print(f"\n📈 Generando gráficas t-SNE para los {len(df_top_models)} modelos con mejor Score...")
        ...
        for i, row in df_top_models.reset_index(drop=True).iterrows():
            ...
            X_tsne = embedding_matrices[emb_key]
            n_dims = X_tsne.shape[1]
            ...
            df_plot = pd.DataFrame(X_tsne, columns=[f'Dim{j+1}' for j in range(n_dims)])
            ...
            if n_dims == 2:
                ...
                ax.set_xlabel('t-SNE Dim1'); ax.set_ylabel('t-SNE Dim2'); ax.set_zlabel('t-SNE Dim3');
                ax.legend(title='Cluster', bbox_to_anchor=(1.05, 1), loc='upper left')
                plt.savefig(filename, bbox_inches='tight')
                plt.close()
                print(f"   > Gráfica 3D guardada: {filename}")

    generate_tsne_plots(df_plot_ready, all_embeddings, OUT_DIR)
...
# 🔧 CORRECCIÓN: Solo intentar merge de clusters si se generaron resultados
if len(df_scores) > 0 and os.path.exists(CLUSTERS_PATH):
    try:
    ...
    print(f"⚠️ No se generaron clusters. El merge final no incluirá información de clustering.")
