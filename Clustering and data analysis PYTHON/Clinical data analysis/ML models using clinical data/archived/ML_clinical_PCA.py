# ============================================================
# 🚀 Pipeline completo: Análisis de clustering Clínico/Demográfico (SOLO PCA)
# ============================================================
...
OUTPUT_DIR = "Results_Clustering_PCA"
...
# =========================
# 6. PCA — ÚNICA ETAPA DE REDUCCIÓN DE DIMENSIONALIDAD
# =========================

max_pca_components = min(X_scaled.shape[0], X_scaled.shape[1])

# Componentes a probar: 2, 3 (para graficar directamente) + dimensionalidades intermedias
PCA_COMPONENTS_TO_TEST = [n for n in [2, 3, 10, 20, 50] if n < max_pca_components]

if len(PCA_COMPONENTS_TO_TEST) == 0:
    PCA_COMPONENTS_TO_TEST = [max_pca_components - 1]

print(f"\n🔸 PCA: Entrada con {X_scaled.shape[1]} features. Componentes a probar: {PCA_COMPONENTS_TO_TEST}")

# Diccionario: n_components -> (X_pca, varianza_acumulada)
pca_outputs = {}

for n_comp in PCA_COMPONENTS_TO_TEST:
    pca_model = PCA(n_components=n_comp, random_state=RANDOM_SEED)
    X_pca = pca_model.fit_transform(X_scaled)
    var_acum = pca_model.explained_variance_ratio_.sum()
    pca_outputs[n_comp] = (X_pca, var_acum)
    print(f"   PCA (n={n_comp}): Varianza acumulada explicada = {var_acum:.4f} ({var_acum*100:.2f}%)")

# Seleccionar automáticamente el PCA que retiene >= 80% de varianza (excluyendo 2 y 3, que son solo para visualización)
PCA_VARIANCE_THRESHOLD = 0.80
pca_intermedias = [n for n in PCA_COMPONENTS_TO_TEST if n > 3]
pca_selected_for_clustering = None

for n_comp in sorted(pca_intermedias):
    if pca_outputs[n_comp][1] >= PCA_VARIANCE_THRESHOLD:
        pca_selected_for_clustering = n_comp
        break

if pca_selected_for_clustering is None and len(pca_intermedias) > 0:
    pca_selected_for_clustering = max(pca_intermedias)
    print(f"   ⚠️ Ningún PCA intermedio alcanzó el umbral de {PCA_VARIANCE_THRESHOLD*100:.0f}% de varianza. Se usará PCA n={pca_selected_for_clustering} (varianza: {pca_outputs[pca_selected_for_clustering][1]*100:.2f}%).")
elif pca_selected_for_clustering is None:
    # Edge case: solo hay 2 o 3 componentes posibles
    pca_selected_for_clustering = max(PCA_COMPONENTS_TO_TEST)
    print(f"   ⚠️ Dataset muy pequeño. Se usará PCA n={pca_selected_for_clustering}.")
else:
    print(f"   ✅ PCA seleccionado para clustering: n={pca_selected_for_clustering} (varianza acumulada: {pca_outputs[pca_selected_for_clustering][1]*100:.2f}%)")

print(f"\n📐 Resumen de espacios PCA que entran al pipeline:")
print(f"   • Clustering principal: PCA n={pca_selected_for_clustering} ({pca_outputs[pca_selected_for_clustering][0].shape[0]} muestras x {pca_outputs[pca_selected_for_clustering][0].shape[1]} componentes, varianza: {pca_outputs[pca_selected_for_clustering][1]*100:.2f}%)")
print(f"   • Graficación 2D: PCA n=2 {'(disponible)' if 2 in pca_outputs else '(no disponible)'}")
print(f"   • Graficación 3D: PCA n=3 {'(disponible)' if 3 in pca_outputs else '(no disponible)'}")


# =========================
# 7. EVALUACIÓN DE CLUSTERING (SETUP Y FUNCIÓN) 
# =========================
...
# =======================================================
# 8. PIPELINE PRINCIPAL: CLUSTERING SOBRE CADA PCA
# =======================================================
...
# Iterar sobre CADA dimensionalidad PCA como espacio de clustering
for pca_n_comp, (X_pca, var_acum) in pca_outputs.items():
    emb_name = f"PCA{pca_n_comp}_var{var_acum*100:.1f}pct"
    print(f"\n🔹 Clustering en espacio: {emb_name} ({X_pca.shape[1]} componentes)")

    results, summary_df = clustering_evaluation_table(X_pca, alg_classes, param_grids, seeds_list=SEEDS_TO_TEST)
    ...
    for index, row in summary_df.iterrows():
        ...
        full_name = f"{row['Algorithm']}_{emb_name}_K{row['Clusters']}_{score_str}{seed_str}"

        all_clustering_results.append({
            'Configuration': full_name, 
            'Algorithm': row['Algorithm'], 
            'Embedding': emb_name,
            'PCA_Components': pca_n_comp,
...
# ------------------------------------------------------------------------
# 10. GRAFICACIÓN DE LOS TOP N MODELOS USANDO PCA 2D y 3D
# ------------------------------------------------------------------------
# Estrategia: cada modelo tiene sus etiquetas de clustering.
# Para visualizarlo, proyectamos las etiquetas sobre PCA-2D y PCA-3D
# (espacios que sí permiten graficar), independientemente de en qué
# espacio PCA se produjo el clustering.

df_top_to_plot = df_selected_models.head(5).copy()

def generate_pca_plots(df_top_models, pca_outputs, output_dir):
    if df_top_models.empty:
        return
    
    print(f"\n📈 Generando gráficas PCA para los {len(df_top_models)} modelos con mejor Score...")

    # Obtener las proyecciones 2D y 3D (si existen)
    X_pca_2d = pca_outputs.get(2, (None, None))[0]
    X_pca_3d = pca_outputs.get(3, (None, None))[0]

    for i, row in df_top_models.reset_index(drop=True).iterrows():
        labels = row['Labels']
        config_name = row['Configuration']
        pca_source = row['PCA_Components']

        title_base = f"TOP {i+1}: {row['Algorithm']} | PCA_source={pca_source}\nSilhouette: {row['Silhouette Score']:.3f} | K={int(row['Clusters'])} | Ruido: {row['Noise_Pct']:.1f}%"

        # --- Preparar paleta de colores ---
        unique_labels = sorted(set(labels.astype(str)))
        cluster_labels_for_palette = [c for c in unique_labels if c != '-1']
        ...
        if X_pca_2d is not None:
            ...
            var_2d = pca_outputs[2][1]
            title_2d = f"{title_base}\n[Proyección: PCA-2D, Var. explicada: {var_2d*100:.1f}%]"
            filename_2d = os.path.join(output_dir, f"TOP_{i+1}_2D_{config_name}.png")

            plt.figure(figsize=(10, 8))
            sns.scatterplot(x='PC1', y='PC2', hue='Cluster', data=df_plot_2d, palette=color_map, legend="full", alpha=0.7, s=50)
            plt.title(title_2d, fontsize=11)
            plt.xlabel(f"PC1 ({pca_outputs[2][1]*100:.1f}% var total acum.)")  # nota: es acumulada, no individual
            plt.ylabel("PC2")
            plt.savefig(filename_2d, bbox_inches='tight')
            plt.close()
            print(f"   > Gráfica 2D guardada: {filename_2d}")
        ...
        if X_pca_3d is not None:
            ...
            var_3d = pca_outputs[3][1]
            title_3d = f"{title_base}\n[Proyección: PCA-3D, Var. explicada: {var_3d*100:.1f}%]"
            filename_3d = os.path.join(output_dir, f"TOP_{i+1}_3D_{config_name}.png")

            fig = plt.figure(figsize=(12, 10))
            ax = fig.add_subplot(111, projection='3d')
            for cluster_label in unique_labels:
                df_subset = df_plot_3d[df_plot_3d['Cluster'] == cluster_label]
                ax.scatter(
                    df_subset['PC1'], df_subset['PC2'], df_subset['PC3'],
                    label=f'Cluster {cluster_label}',
                    color=color_map.get(cluster_label, 'black'),
                    alpha=0.7, s=50
                )
            ax.set_title(title_3d, fontsize=11)
            ax.set_xlabel('PC1')
            ax.set_ylabel('PC2')
            ax.set_zlabel('PC3')
            ax.legend(title='Cluster', bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.savefig(filename_3d, bbox_inches='tight')
            plt.close()
            print(f"   > Gráfica 3D guardada: {filename_3d}")

        if X_pca_2d is None and X_pca_3d is None:
            print(f"   ⚠️ No hay proyección PCA-2D ni PCA-3D disponible para graficar.")

generate_pca_plots(df_top_to_plot, pca_outputs, OUTPUT_DIR)
