...
# Features: Flujos crudos + todas las métricas secundarias
# ============================================================
...
# ============================================================
PATH_FEATURES = "/Users/eduardoruiz/Documents/GitHub/Precision-Oncology-for-Breast-Cancer-Diagnosis/Clinical_data_and_models_ids/FeatureMatrix_TumorPhenotype.csv"   # ← CAMBIAR
...
OUT_DIR       = "resultados_TumorPhenotype"
...
ZERO_NULL_THRESHOLD  = 0.95   # eliminar columnas con >95% ceros/NaN
SILHOUETTE_THRESHOLD = 0.10   # umbral mínimo para conservar clustering
MAX_NOISE_PCT        = 5.0    # ruido máximo tolerado (DBSCAN/HDBSCAN)

# Nombres exactos de las 3 soluciones (deben coincidir con el CSV)
SOL_NAMES = ["FBA", "pFBA", "L1w"]

# ============================================================
# 🔧 RAÍCES DE MÉTRICAS SECUNDARIAS
#    NOTA: matlab.lang.makeValidName reemplaza '/' por 'x2F' y
#    antepone 'x' a nombres que empiezan con número (ej. '2HG'→'x2HG').
#    Si tu CSV tiene nombres distintos, ajusta aquí.
# ============================================================
METRIC_ROOTS = [
    # --- Originales ---
    "CU",
    ...
    # --- Subsystem Activity ---
    "SA_TCA_cycle",
    "SA_Glycolysis___Gluconeogenesis",     # ' / ' → '___' por makeValidName
    "SA_Fatty_acid_synthesis",
    "SA_Purine_metabolism",
    "SA_Glutamate_metabolism",
    "SA_Oxidative_phosphorylation",
    # --- Nuevas métricas ---
    "RedoxIndex",
    "MFI",
    "AnabolismScore",
...
# Oncometabolitos: '2HG' → 'x2HG' por makeValidName
ONCOMET_NAMES = ["Lactate", "Succinate", "Fumarate", "x2HG"]

# ============================================================
# 🔑 UTILIDADES
# ============================================================
def extract_model_id(model_name: str) -> str:
    """Extrae ID TCGA estándar de 16 chars del nombre del modelo."""
    match = re.search(
...
def detect_columns(df: pd.DataFrame) -> dict:
    """
    Detecta automáticamente qué columnas del CSV corresponden a cada
    tipo de feature, usando los SOL_NAMES, METRIC_ROOTS y ONCOMET_NAMES
    definidos globalmente. Imprime advertencias para raíces no encontradas.
    """
    ...
    # Oncometabolitos: Oncomet_<met>_<sol>
...
# ============================================================
# 1️⃣  CARGA Y VALIDACIÓN DEL FEATURE MATRIX
# ============================================================
print("\n" + "="*60)
...
# Normalizar columna Model → ID TCGA de 16 chars
...
# Diagnóstico rápido de columnas (útil para ajustar METRIC_ROOTS)
...
# ── MODO de features ──────────────────────────────────────────────────────
# "secondary" → solo métricas resumen  (interpretable, recomendado inicio)
# "flux"      → solo flujos crudos     (alta dimensión, más lento)
# "all"       → todo combinado         (máximo poder, requiere más RAM)
FEATURE_MODE = "all"   # ← CAMBIA AQUÍ
...
if not feature_cols:
    raise ValueError(
        ...
        "Revisa METRIC_ROOTS, SOL_NAMES y los nombres exactos del CSV."
    )
...
# Imputar NaN con mediana (preserva distribución)
imputer = SimpleImputer(strategy="median")
X_imp   = imputer.fit_transform(X_raw)

# Eliminar columnas constantes post-imputación
...
# RobustScaler: resistente a outliers de flujos metabólicos
scaler   = RobustScaler()
...
# ============================================================
# 5️⃣  PCA → UMAP
#     PCA reduce de ~6884 → N_PCA componentes antes de UMAP.
#     Esto baja el tiempo de horas a minutos sin perder
#     información relevante (los primeros 50 PCs capturan la
#     mayoría de la varianza en datos de flujos metabólicos).
# ============================================================
print("\n" + "="*60)
print("🔻 5. PCA + REDUCCIÓN UMAP")
print("="*60)

from sklearn.decomposition import PCA

# ── Parámetro ajustable ───────────────────────────────────
N_PCA = 100   # ← Aumenta a 100 si quieres más detalle (más lento)
             #   Baja a 30 si tu máquina es lenta
# ─────────────────────────────────────────────────────────

N_PCA = min(N_PCA, X_scaled.shape[0], X_scaled.shape[1])

print(f"   Dimensiones entrada PCA : {X_scaled.shape[1]}")
print(f"   Componentes PCA         : {N_PCA}")

pca      = PCA(n_components=N_PCA, random_state=RANDOM_SEED)
X_pca    = pca.fit_transform(X_scaled)
var_acum = pca.explained_variance_ratio_.cumsum()

print(f"   Varianza acumulada PC{N_PCA:02d} : {var_acum[-1]*100:.1f}%")
print(f"   Varianza acumulada PC10  : {var_acum[9]*100:.1f}%")
print(f"   Varianza acumulada PC20  : {var_acum[19]*100:.1f}%")
print(f"   ✅ X_pca: {X_pca.shape}")

# Guardar gráfica de varianza explicada
fig, ax = plt.subplots(figsize=(8, 4))
ax.plot(range(1, N_PCA + 1), var_acum * 100, marker="o", markersize=3, linewidth=1.5)
...
ax.set_title("Varianza explicada acumulada — PCA pre-UMAP")
ax.legend()
ax.grid(alpha=0.3)
plt.tight_layout()
pca_plot = os.path.join(OUT_DIR, "PCA_varianza_acumulada.png")
plt.savefig(pca_plot, dpi=150, bbox_inches="tight")
plt.close()
print(f"   📊 Gráfica PCA guardada: {pca_plot}")

# ── Configuraciones UMAP (sobre X_pca) ───────────────────
# cosine se omite porque PCA ya ortogonaliza el espacio
umap_configs = [
    {"n_components": nc, "n_neighbors": nn, "min_dist": md, "metric": mt}
    for nc in [2, 3]
    for nn in [10, 30, 50]
    for md in [0.05, 0.3]
    for mt in ["euclidean", "manhattan"]
]

total_emb = len(umap_configs) * len(SEEDS_TO_TEST)
print(f"\n   Configs UMAP : {len(umap_configs)} × {len(SEEDS_TO_TEST)} semillas = {total_emb} embeddings")
print(f"   Input UMAP   : X_pca {X_pca.shape}  (antes X_scaled {X_scaled.shape})")

embedding_matrices = {}
for seed in SEEDS_TO_TEST:
    np.random.seed(seed)
    for params in umap_configs:
        name = (f"UMAP_C{params['n_components']}_NN{params['n_neighbors']}"
                f"_MD{params['min_dist']}_M{params['metric']}_S{seed}")
        try:
            X_umap = umap.UMAP(**params, random_state=seed).fit_transform(X_pca)
            embedding_matrices[name] = X_umap
        except Exception as e:
            print(f"   ⚠️  Error en {name}: {e}")

print(f"   ✅ Embeddings generados: {len(embedding_matrices)}")
...
# Algoritmos deterministas (solo necesitan una semilla)
...
# ============================================================
# 9️⃣  FILTRADO Y EXPORTACIÓN DE CLUSTERINGS
# ============================================================
print("\n" + "="*60)
...
print(f"   Clusterings seleccionados (Sil ≥ {SILHOUETTE_THRESHOLD}): {len(df_selected)}")

# Construir DataFrame de asignaciones de cluster por paciente
...
for i, row in df_selected.iterrows():
    ...
    if i < 10:
        print(f"   {i+1:2d}. {row['reduction']:50s} | "
              ...
              f"Sil={row['score']:.4f} | Seed={row['seed']}")
    elif i == 10:
        print("   ... (listado truncado) ...")

CLUSTERS_PATH = os.path.join(OUT_DIR, "PatientClusters_TumorPhenotype.csv")
df_clusters_final.to_csv(CLUSTERS_PATH, index=False)
print(f"\n   ✅ Clusters guardados: {CLUSTERS_PATH}")
...
def generate_umap_plots(df_top: pd.DataFrame,
    ...
    for i, row in df_top.head(top_n).reset_index(drop=True).iterrows():
        ...
        labels   = np.asarray(row["labels"])
        config   = (f"{row['algorithm']}_K{row['Optimal K/Comp']}"
                    f"_S{row['score']:.2f}_Seed{row['seed']}")
        title    = (f"TOP {i+1}: {emb_key} + {config}\n"
        ...
        X_umap   = emb_dict[emb_key]
        n_dims   = X_umap.shape[1]
        df_plot  = pd.DataFrame(X_umap,
                                columns=[f"Dim{j+1}" for j in range(n_dims)])
        df_plot["Cluster"] = labels.astype(str)

        unique_cl = sorted(df_plot["Cluster"].unique())
        ...
        if n_dims == 2:
            ...
            sns.scatterplot(x="Dim1", y="Dim2", hue="Cluster",
                            data=df_plot, palette=cmap,
                            legend="full", alpha=0.7, s=50, ax=ax)
            ...
            for cl in unique_cl:
                ...
                ax.scatter(sub["Dim1"], sub["Dim2"], sub["Dim3"],
                           label=f"Cluster {cl}",
                           color=cmap.get(cl, "black"), alpha=0.7, s=40)
...
def load_clinical(path: str) -> pd.DataFrame:
    """Carga TSV clínico/survival y normaliza la columna Model."""
    try:
...
# Base de merge: Model + PatientID sin features (más ligero)
base    = df_raw[["Model","PatientID"]].drop_duplicates(subset="Model").copy()
valid_m = set(base["Model"])

merged = base.copy()
for right_df, name in [(df_clinical, "Clinical"), (df_survival, "Survival")]:
    filt = right_df[right_df["Model"].isin(valid_m)]
    if not filt.empty:
        merged = merged.merge(filt.drop_duplicates(subset="Model"),
                              on="Model", how="left")
        print(f"   ✅ Merge {name}: {len(filt)} registros")
    else:
        print(f"   ⚠️  {name} vacío o sin coincidencias")

# Añadir features metabólicos
merged_feat = merged.merge(df_features, on="Model", how="left")

OUT_BASE = os.path.join(OUT_DIR, "Merged_TumorPhenotype_clinical.csv")
merged_feat.to_csv(OUT_BASE, index=False)
print(f"\n   ✅ Dataset clínica + features: {OUT_BASE}")

# Añadir asignaciones de cluster
try:
    ...
    OUT_FINAL = os.path.join(OUT_DIR, "Merged_TumorPhenotype_withClusters.csv")
    merged_final.to_csv(OUT_FINAL, index=False)
    print(f"   ✅ Dataset final con clusters: {OUT_FINAL}")

    preview = ["Model"] + cluster_cols[:3]
    print("\n   Preview (primeros 3 clusterings):")
    print(merged_final[preview].head(8).to_string(index=False))
...
print(f"     ↳ Flujos crudos            : {len(col_groups['flux'])}")
print(f"     ↳ Métricas secundarias     : {len(col_groups['secondary'])}")
print(f"   Cols eliminadas (limpieza)   : {len(cols_to_drop)}")
print(f"   Embeddings UMAP              : {len(embedding_matrices)}")
print(f"   Clusterings evaluados        : {len(rows)}")
print(f"   Clusterings seleccionados    : {len(df_selected)} (Sil ≥ {SILHOUETTE_THRESHOLD})")
