...
umap_configs = [
    ...
    for nn in [10, 30, 50]
    for md in [0.05, 0.3]
    for mt in ["euclidean", "manhattan", "cosine"]
]

total_emb = len(umap_configs) * len(SEEDS_TO_TEST)
print(f"   Configs UMAP : {len(umap_configs)} × {len(SEEDS_TO_TEST)} semillas = {total_emb} embeddings")
...
for seed in SEEDS_TO_TEST:
    ...
    for params in umap_configs:
        ...
        try:
            X_umap = umap.UMAP(**params, random_state=seed).fit_transform(X_scaled)
