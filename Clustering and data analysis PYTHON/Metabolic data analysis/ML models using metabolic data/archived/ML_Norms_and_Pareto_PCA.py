...
from sklearn.decomposition import PCA  # 🔄 CAMBIO: PCA en lugar de UMAP
...
OUT_DIR = "resultados_normas_pareto_PCA" # Directorio de salida actualizado
...
# ============================================================
# 4️⃣ Reducción PCA con Múltiples Semillas 🔑
# ============================================================
embedding_matrices = {} 
X_input = X_scaled

# 🔄 CAMBIO: Parámetros para PCA
n_components_list = [2, 3]

print(f"\n🔸 Ejecutando PCA en X_scaled ({X_scaled.shape[1]} features) con {len(n_components_list)} configuraciones y {len(SEEDS_TO_TEST)} semillas...")

for current_seed in SEEDS_TO_TEST:
    np.random.seed(current_seed) 
    for n_components in n_components_list:
        emb_name = f'PCA_C{n_components}_S{current_seed}'

        try:
            pca_model = PCA(n_components=n_components, random_state=current_seed)
            X_pca = pca_model.fit_transform(X_input)
            embedding_matrices[emb_name] = X_pca
            
            # Reportar varianza explicada
            variance_explained = pca_model.explained_variance_ratio_.sum() * 100
            print(f"   ✓ {emb_name}: {variance_explained:.2f}% varianza explicada")
            
        except Exception as e:
            print(f"Error al ejecutar PCA con {emb_name}: {e}")
            continue
...
# ============================================================
# 12️⃣ PLOTEO DEL MEJOR PCA 2D/3D 🖼️
# ============================================================
...
def generate_pca_plots(df_top_models, embedding_matrices, output_dir):
    """Genera y muestra las gráficas de PCA para un subconjunto de los modelos."""
    ...
    for i, row in df_top_models.reset_index(drop=True).iterrows():
        ...
        if n_dims == 2:
            ...
            sns.scatterplot(x='PC1', y='PC2', hue='Cluster', data=df_plot, palette=color_map, legend="full", alpha=0.7, s=50)
            ...
            for cluster_label in cluster_list:
                ...
                ax.scatter(df_subset['PC1'], df_subset['PC2'], df_subset['PC3'], label=f'Cluster {cluster_label}', color=color_map.get(cluster_label, 'black'), alpha=0.7, s=50)
...
generate_pca_plots(df_plot_ready, all_embeddings, OUT_DIR)
