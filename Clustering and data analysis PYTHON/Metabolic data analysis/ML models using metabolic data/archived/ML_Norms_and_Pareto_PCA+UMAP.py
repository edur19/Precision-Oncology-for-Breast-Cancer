...
from sklearn.decomposition import PCA  # 🔄 CAMBIO: PCA para primera reducción
import umap  # 🔄 CAMBIO: UMAP para segunda reducción
...
OUT_DIR = "resultados_normas_pareto_PCA+UMAP" # Directorio de salida actualizado
...
# ============================================================
# 4️⃣ Reducción PCA+UMAP con Múltiples Semillas y Grid Search 🔑
# ============================================================
embedding_matrices = {} 
X_input = X_scaled

# 🔄 CAMBIO: Parámetros para reducción en dos etapas (PCA → UMAP)
pca_components_list = [20, 30, 50]  # Componentes intermedios de PCA
umap_components_list = [2, 3]  # Componentes finales de UMAP
n_neighbors_list = [10, 30, 50]
min_dist_list = [0.05, 0.3]
metric_list = ['euclidean', 'manhattan', 'cosine']

print(f"\n🔸 Ejecutando reducción PCA+UMAP en X_scaled ({X_scaled.shape[1]} features) con múltiples configuraciones y {len(SEEDS_TO_TEST)} semillas...")

for current_seed in SEEDS_TO_TEST:
    np.random.seed(current_seed)
    
    # ETAPA 1: Reducción con PCA
    pca_embeddings = {}
    for pca_comp in pca_components_list:
        # Ajustar si hay menos muestras que componentes
        pca_comp_actual = min(pca_comp, X_input.shape[0] - 1, X_input.shape[1])
        
        try:
            pca_model = PCA(n_components=pca_comp_actual, random_state=current_seed)
            X_pca = pca_model.fit_transform(X_input)
            variance_explained = pca_model.explained_variance_ratio_.sum() * 100
            
            pca_key = f'PCA{pca_comp_actual}_S{current_seed}'
            pca_embeddings[pca_key] = X_pca
            print(f"   ✓ PCA ({pca_comp_actual} componentes): {variance_explained:.2f}% varianza explicada")
            
        except Exception as e:
            print(f"   ✗ Error en PCA con {pca_comp_actual} componentes: {e}")
            continue
    
    # ETAPA 2: Aplicar UMAP sobre cada embedding de PCA
    for pca_key, X_pca in pca_embeddings.items():
        for umap_comp in umap_components_list:
            for n_neigh in n_neighbors_list:
                for m_dist in min_dist_list:
                    for metric_type in metric_list:
                        emb_name = f'{pca_key}_UMAP_C{umap_comp}_NN{n_neigh}_MD{m_dist}_M{metric_type}'
                        
                        try:
                            umap_model = umap.UMAP(
                                n_components=umap_comp,
                                n_neighbors=n_neigh,
                                min_dist=m_dist,
                                metric=metric_type,
                            ...
                            print(f"   ✗ Error en UMAP para {emb_name}: {e}")
                            continue

all_embeddings = {
    **embedding_matrices
}
print(f"✅ Embeddings para clustering generados: {len(all_embeddings)} configuraciones (PCA+UMAP)")
...
# ============================================================
# 12️⃣ PLOTEO DEL MEJOR PCA+UMAP 2D/3D 🖼️
# ============================================================
...
def generate_pca_umap_plots(df_top_models, embedding_matrices, output_dir):
    """Genera y muestra las gráficas de PCA+UMAP para un subconjunto de los modelos."""

    if df_top_models.empty:
        print("\n⚠️ No hay modelos TOP para graficar.")
        return

    print(f"\n📈 Generando gráficas PCA+UMAP para los {len(df_top_models)} modelos con mejor Score...")
    ...
    for i, row in df_top_models.reset_index(drop=True).iterrows():
        ...
        X_embedding = embedding_matrices[emb_key]
        n_dims = X_embedding.shape[1]
        ...
        df_plot = pd.DataFrame(X_embedding, columns=[f'Dim{j+1}' for j in range(n_dims)])
        ...
        if n_dims == 2:
            ...
            ax.set_xlabel('UMAP Dim1'); ax.set_ylabel('UMAP Dim2'); ax.set_zlabel('UMAP Dim3');
            ax.legend(title='Cluster', bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.savefig(filename, bbox_inches='tight')
            plt.close()
            print(f"   > Gráfica 3D guardada: {filename}")

generate_pca_umap_plots(df_plot_ready, all_embeddings, OUT_DIR)
