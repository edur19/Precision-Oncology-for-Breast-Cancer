...
from sklearn.decomposition import PCA  # ✨ AGREGADO PARA PCA
import umap
...
OUT_DIR = "resultados_pareto_cleaned_PCA" # Directorio de salida actualizado
...
# ============================================================
# 3.5️⃣ ✨ NUEVO: Aplicar PCA antes de UMAP 🔑
# ============================================================
# Configuración de PCA
PCA_N_COMPONENTS = 0.95  # Retener 95% de la varianza explicada
# Alternativa: usar un número fijo, por ejemplo: PCA_N_COMPONENTS = 50

print(f"\n🔸 Aplicando PCA para reducir dimensionalidad antes de UMAP...")
print(f"   Configuración: n_components={PCA_N_COMPONENTS} (varianza explicada)")
...
print(f"   • Dimensiones después de PCA: {n_components_pca}")
print(f"   • Varianza explicada total: {variance_explained:.2%}")
print(f"   • Reducción de dimensionalidad: {(1 - n_components_pca/X_scaled.shape[1]):.1%}")

# Guardar información de PCA
pca_info = {
    'n_components_original': X_scaled.shape[1],
    'n_components_pca': n_components_pca,
    'variance_explained': variance_explained,
    'variance_per_component': pca.explained_variance_ratio_.tolist()
}

# ✨ CORRECCIÓN: Crear índice de features compatible con las dimensiones de PCA
actual_n_features = pca.components_.shape[1]  # Número de features que PCA realmente usó
print(f"   • Features en PCA: {actual_n_features}, Features en feature_cols: {len(feature_cols)}")

# Crear un índice seguro para el DataFrame
if len(feature_cols) == actual_n_features:
    feature_index = feature_cols
else:
    print(f"⚠️ Desajuste detectado. Usando índice numérico genérico.")
    # Usar los primeros N nombres si están disponibles, sino crear genéricos
    feature_index = []
    for i in range(actual_n_features):
        if i < len(feature_cols):
            feature_index.append(feature_cols[i])
        else:
            feature_index.append(f'Feature_{i}')

# Opcional: Guardar los componentes principales
pca_components_df = pd.DataFrame(
    pca.components_.T,
    columns=[f'PC{i+1}' for i in range(n_components_pca)],
    index=feature_index
)
pca_loadings_path = os.path.join(OUT_DIR, "PCA_feature_loadings.csv")
pca_components_df.to_csv(pca_loadings_path)
print(f"   • Cargas de features guardadas en: {pca_loadings_path}")

# Guardar la varianza explicada por componente
variance_df = pd.DataFrame({
    'Component': [f'PC{i+1}' for i in range(n_components_pca)],
    'Variance_Explained': pca.explained_variance_ratio_,
    'Cumulative_Variance': np.cumsum(pca.explained_variance_ratio_)
})
variance_path = os.path.join(OUT_DIR, "PCA_variance_explained.csv")
variance_df.to_csv(variance_path, index=False)
print(f"   • Varianza explicada guardada en: {variance_path}")
...
embedding_matrices = {} 
X_input = X_pca  # ✨ CAMBIADO: Ahora usamos X_pca en lugar de X_scaled
...
print(f"\n🔸 Ejecutando UMAP en X_pca ({X_pca.shape[1]} componentes PCA) con {len(umap_param_grid)} configuraciones y {len(SEEDS_TO_TEST)} semillas...")
...
CLUSTERS_PATH = os.path.join(OUT_DIR, "PatientClusters_Selected_PARETO_cleaned_PCA.csv")
...
OUT_PATH = os.path.join(OUT_DIR, "Merged_TCGA_BRCA_AllData_safe_PARETO_cleaned_PCA.csv")
...
try:
    ...
    OUT_UPDATED = os.path.join(OUT_DIR, "Merged_TCGA_BRCA_AllData_safe_withSelectedClusters_PARETO_cleaned_PCA.csv")
...
# ============================================================
# ✨ BONUS: Gráfica de Varianza Explicada por PCA
# ============================================================
print("\n📊 Generando gráfica de varianza explicada por PCA...")
...
plt.xticks(range(1, n_components_pca + 1, max(1, n_components_pca // 10)))

# Subplot 2: Varianza acumulada
plt.subplot(1, 2, 2)
plt.plot(range(1, n_components_pca + 1), np.cumsum(pca.explained_variance_ratio_), marker='o')
...
plt.tight_layout()
variance_plot_path = os.path.join(OUT_DIR, "PCA_variance_explained.png")
plt.savefig(variance_plot_path, dpi=300, bbox_inches='tight')
plt.close()
print(f"✅ Gráfica de varianza guardada en: {variance_plot_path}")
...
print("="*60)
print(f"Columnas originales (Pareto): {len(agg_dict_pareto)}")
if cols_to_drop:
