# ============================================================
# 🚀 Pipeline completo: Análisis de clustering Clínico/Demográfico (CON PCA + UMAP)
# ============================================================
...
OUTPUT_DIR = "Results_clustering_PCA+UMAP"
...
# =========================
# 5.5 PCA — REDUCCIÓN DE DIMENSIONALIDAD PREVIA A UMAP
# =========================

# Determinar el número máximo de componentes posibles
max_pca_components = min(X_scaled.shape[0], X_scaled.shape[1])

# Lista de componentes PCA a probar (se limita automáticamente al máximo permitido)
PCA_COMPONENTS_TO_TEST = [n for n in [10, 20, 50] if n < max_pca_components]

# Si ningún valor cabe (dataset muy pequeño), usar el máximo - 1
...
# Diccionario para almacenar las salidas PCA
pca_outputs = {}  # clave: n_components -> valor: (X_pca, varianza_explicada_acumulada)
...
# Seleccionar automáticamente el PCA que retiene >= 80% de varianza.
# Si ninguno alcanza el 80%, usar el de mayor dimensionalidad.
PCA_VARIANCE_THRESHOLD = 0.80
pca_selected = None
for n_comp in sorted(PCA_COMPONENTS_TO_TEST):
    if pca_outputs[n_comp][1] >= PCA_VARIANCE_THRESHOLD:
        pca_selected = n_comp
        break

if pca_selected is None:
    pca_selected = max(PCA_COMPONENTS_TO_TEST)
    print(f"   ⚠️ Ningún PCA alcanzó el umbral de {PCA_VARIANCE_THRESHOLD*100:.0f}% de varianza. Se usará PCA n={pca_selected} (varianza: {pca_outputs[pca_selected][1]*100:.2f}%).")
else:
    print(f"   ✅ PCA seleccionado automáticamente: n={pca_selected} (varianza acumulada: {pca_outputs[pca_selected][1]*100:.2f}%)")

X_pca_final = pca_outputs[pca_selected][0]
print(f"   📐 Forma de la matriz PCA de entrada a UMAP: {X_pca_final.shape}")
...
# =======================================================
# 7. GENERAR EMBEDDINGS UMAP (ENTRADA: X_pca_final)
# =======================================================
embedding_results = {}
embedding_matrices = {}

# ✅ ENTRADA AL UMAP: Ahora es la salida del PCA seleccionado
X_input = X_pca_final
...
print(f"\n🔸 Ejecutando UMAP en X_pca_final ({X_input.shape[1]} componentes PCA) con {len(umap_param_grid)} configs y {len(SEEDS_TO_TEST)} semillas...")
...
for current_seed in SEEDS_TO_TEST:
    ...
    for params in umap_param_grid:
        ...
        emb_name = f'PCA{pca_selected}_UMAP_C{n_components}_NN{n_neighbors}_MD{min_dist}_M{metric_name}_S{current_seed}'
        ...
        try:
            ...
            embedding_results[emb_name] = X_umap
        except Exception as e:
