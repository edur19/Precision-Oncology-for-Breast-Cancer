# ============================================================
# 🚀 Pipeline completo: Análisis de clustering metabólico BRCA (Normas + Pareto - Soluciones Individuales)
# ============================================================
# =========================
...
PATH_PARETO = "/Users/eduardoruiz/Documents/MCBCI/MCBCI2/Sistemas metabólicos/Proyecto_Tesis/Datos_actual/ParetoSurface_CU_EA_extended_1226_Final (1).csv" 
...
OUT_DIR = "resultados_de_normas_PARETO_Wide_sin_filtro" # Directorio de salida actualizado
...
# --- 2.3 Carga y TRANSFORMACIÓN WIDE de Pareto (SOLUCIONES INDIVIDUALES) 🔑 ---
try:
...
if PARETO_LOADED:
    ...
    feature_cols_pareto_wide = [col for col in df_pareto_wide.columns if col != 'Model']
    print(f"✅ Features de Pareto WIDE creados: {len(feature_cols_pareto_wide)} columnas (Soluciones Individuales).")
    
    # --- MERGE DE FEATURES: Normas (LEFT) + Pareto WIDE ---
    ...
    feature_cols = feature_cols_normas + feature_cols_pareto_wide
...
print(f"\n⚠️ FILTRO ELIMINADO: Se utiliza la base combinada (Normas + Pareto WIDE) con {len(patient_ids)} modelos.")
...
# Reemplazar 0 por NaN en Normas 1/2 y luego imputar con mediana (para robustez y manejar NaN de Left Join)
...
# 3.2. Aplicar Winsorización (a toda la matriz de features combinada)
...
CLUSTERS_PATH = os.path.join(OUT_DIR, "PatientClusters_Selected_NormasPareto_sinfiltro.csv")
...
OUT_PATH = os.path.join(OUT_DIR, "Merged_TCGA_BRCA_AllData_safe_NormasPareto_sinfiltro.csv")
...
try:
    ...
    OUT_UPDATED = os.path.join(OUT_DIR, "Merged_TCGA_BRCA_AllData_safe_withSelectedClusters_NormasPareto_sinfiltro.csv")
